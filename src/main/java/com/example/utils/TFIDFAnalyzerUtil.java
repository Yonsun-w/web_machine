 ...
 */
public  class TFIDFAnalyzerUtil
{
     ...
     * @return
     */
    public static List<Keyword> analyze(String content, int topN) throws Exception {
        List<Keyword> keywordList=new ArrayList<>();

        if(stopWordsSet==null) {
            stopWordsSet=new HashSet<>();
            ClassPathResource classPathResource = new ClassPathResource("static/jieba/stop_words.txt");
            InputStream input =classPathResource.getInputStream();
            byte data[] = new byte[1024];
            //4:读取数据
            input.read(data); //将数据读取到字节数组中
      loadStopWords(stopWordsSet, input);
        }
        if(idfMap==null) {
            idfMap=new HashMap<>();
            ClassPathResource classPathResource = new ClassPathResource("static/jieba/idf_dict.txt");
            InputStream input =classPathResource.getInputStream();

            byte data[] = new byte[1024];
                loadIDFMap(idfMap, input);
        }
...
//	{
//		String content="美味陶瓷分析中国的";
//		int topN=5;
//		TFIDFAnalyzer tfidfAnalyzer=new TFIDFAnalyzer();
//		List<Keyword> list=tfidfAnalyzer.analyze(content,3);
